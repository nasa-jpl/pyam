...
# --------------------------------------------------------------------------------
print("====== MODIFYING MyNewGitModule R1-00 branch  - add hey.txt")
...
# --------------------------------------------------------------------------------
print("====== SAVING MyNewGitModule R1-00 branch as R1-00a release ")
...
print("Repo branches=")
os.system(f"git -C {repo_dir} branch")


print("Release tag=")
os.system(f"git -C {new_release_dir} describe --tags")

print("Release branch=")
os.system(f"git -C {new_release_dir} branch")
os.system(f"git -C {new_release_dir} status -b")


print("DONE SAVING R1-00a, CHECKING OUT R1-00a branch")

# --------------------------------------------------------------------------------
print("====== CHECKOUT SAVED MyNewGitModule R1-00a branch ")
...
os.system(f"git -C {new_mod_dir} branch")
os.system(f"git -C {new_mod_dir} status -b")

# --------------------------------------------------------------------------------
print("====== MODIFY MyNewGitModule R1-00a MODULE - add heythere.txt ")
...
# --------------------------------------------------------------------------------
print("====== RE-SAVE  MyNewGitModule MODULE as R1-00b release")
...
# --------------------------------------------------------------------------------
print("====== CHECKOUT MyNewGitModule new R1-00a branch ")

os.system(f"{str(pyam)} {options_str} scrap --remove MyNewGitModule")

# check_cmd = f'{str(pyam)} {options_str} {checkout} --release R1-00a --branch syncThisBranch'
check_cmd = f"{str(pyam)} {options_str} {checkout} --release R1-00a"
...
# --------------------------------------------------------------------------------
print("====== MODIFY MyNewGitModule R1-00a new branch - add heythere2.txt ")

# add files to this branch in the sandbox
new_mod_dir = os.path.join(sandbox_dir, "src", "MyNewGitModule")
os.chdir(new_mod_dir)
new_file = os.path.join(new_mod_dir, "heythere2.txt")
with open(new_file, "w") as f:
    f.write("Hey There2")
repo = Repo(new_mod_dir)
repo.git.add("heythere2.txt")
repo.index.commit("Added heythere2.txt to branch")
repo.git.push()
os.chdir(sandbox_dir)

# --------------------------------------------------------------------------------
print("====== SYNCING MyNewGitModule MODULE")

print("Checked out tag before sync=")
os.system(f"git -C {new_mod_dir} describe --tags")
print("Checked out branch before sync=")
os.system(f"git -C {new_mod_dir} status -b")


sync_cmd = f"{str(pyam)} {options_str} sync MyNewGitModule"
os.system(sync_cmd)
