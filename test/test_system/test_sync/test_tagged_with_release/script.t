...
Note that "R4-06f" is one revision behind the latest.
    ...
    $ echo 'BRANCH_Dshell++ = Dshell++-R4-06e' >> "$CRAMTMP/FakeSandbox/YAM.config"
...
Check our current SVN URL.
    ...
    ^/Modules/Dshell++/releases/Dshell++-R4-06e
...
--------------------------------------------------------------
Now we will check that we can individually sync modules to a branch

    $ pushd "$sandbox_directory" >& /dev/null
    $ $PYAM sync DshellEnv --release R1-49r --branch junk
    Syncing DshellEnv work module to R1-49r release
    $ svn status 'src/Dshell++'
...
Make sure our SVN URL is updated to be off the latest revision.
    ...
    ^/Modules/Dshell++/releases/Dshell++-R4-06f
    $ svn info  --show-item relative-url "$sandbox_directory/src/DshellEnv"
    ^/Modules/DshellEnv/featureBranches/DshellEnv-R1-49r-junk
    $ cat "$sandbox_directory/YAM.config" | grep 'Dshell'
    WORK_MODULES = Dshell++ \
                   DshellEnv
    BRANCH_Dshell++  = Dshell++-R4-06f
    BRANCH_DshellEnv = DshellEnv-R1-49r junk
