...
echo 'WORK_MODULES = Darts Dshell++' >> "$config_filename"
echo 'BRANCH_Dshell++ = Dshell++-R4-06g my_branch' >> "$config_filename"
echo 'BRANCH_Darts = Darts-R3-15a my_branch' >> "$config_filename"
...
"$PYAM" --quiet --no-log --no-build-server --database-connection="127.0.0.1:$port/test" config --all-to-link --output-file $config_filename
