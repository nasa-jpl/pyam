...
"$PYAM" --quiet --no-log --release-directory="$release_directory" \
        --no-build-server --database-connection="127.0.0.1:$port/test" \
        --default-repository-url="$fake_repository_url" \
        --keyword-to-repository-url="my_keyword=$fake_repository_url" \
        --release-directory="$release_directory" \
        register-new-module --repository-keyword='my_keyword' 'MyNewModule'
...
"$PYAM" --quiet --no-log --release-directory="$release_directory" \
        --no-build-server --database-connection="127.0.0.1:$port/test" --default-repository-url="$fake_repository_url" --keyword-to-repository-url="my_keyword=$fake_repository_url" rebuild 'MyNewModule'
popd > /dev/null
...
# Save the branch
pushd "$sandbox_directory" >& /dev/null
"$PYAM" --quiet --no-log --non-interactive --release-directory="$release_directory" \
        --no-build-server --database-connection="127.0.0.1:$port/test" --default-repository-url="$fake_repository_url" --keyword-to-repository-url="my_keyword=$fake_repository_url" \
        --release-directory="$release_directory" \
        save 'MyNewModule'
if [ $? -ne 0 ]
then
...
"$PYAM" --quiet --no-log --release-directory="$release_directory" \
        --no-build-server --database-connection="127.0.0.1:$port/test" --default-repository-url="$fake_repository_url" --keyword-to-repository-url="my_keyword=$fake_repository_url" checkout 'MyNewModule'
if [ $? -ne 0 ]
then
