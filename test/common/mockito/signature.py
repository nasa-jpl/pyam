...
def get_signature(obj, method_name):
    ...
    if PY3:
        ...
        if isinstance(method, types.UnboundMethodType) and method.__self__ is None:
            method = functools.partial(method, None)
...
def match_signature_allowing_placeholders(sig, args, kwargs):  # noqa: C901
    ...
    if contains_strict(args, Ellipsis):
        ...
        # bind with the rest.
        params = [p for n, p in sig.parameters.items() if p.kind not in (Parameter.KEYWORD_ONLY, Parameter.VAR_KEYWORD)]
            ...
            try:
                ...
                if kwargs_provided and not has_var_keyword(sig):
                    ...
                    if provided_args + 1 > pos_args:
                        raise TypeError("no keyword argument for **kwargs left")
...
def positional_arguments(sig):
    return len(
        [p for n, p in sig.parameters.items() if p.kind in (Parameter.POSITIONAL_ONLY, Parameter.POSITIONAL_OR_KEYWORD)]
    )


def has_var_keyword(sig):
    return any(p for n, p in sig.parameters.items() if p.kind is Parameter.VAR_KEYWORD)
